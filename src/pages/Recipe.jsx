...
console.log("recipeData",recipeData);
      ...
      setYoutubeId(recipeData[0].strYoutube.split("?v=")[1]);
      sessionStorage.setItem("recipe", JSON.stringify(recipeData));

      checkIfFavorite(recipeData[0].idMeal);
    } catch (error) {
  ...
  const checkIfFavorite = (id) => {
    ...
    setIsFavorite(favorites.some((item) => item.idMeal === id));
  };
  ...
  useEffect(() => {
    if (inputData.trim().length === 0) {
      ...
      if (storedRecipe.length !== 0) {
        setSearchData(storedRecipe);
        setYoutubeId(storedRecipe[0].strYoutube.split("?v=")[1]);
        checkIfFavorite(storedRecipe[0].idMeal);
      } else {
        setErrorMessage("Sorry Recipe Not Found");
      }
    } else {
                  ...
                  >
                    <div className="flex flex-col sm:flex-row text-center sm:text-left justify-center sm:justify-between items-center p-2 w-full gap-5 max-w-screen-lg m-auto">
                      ...
                      <div className="w-1/2 flex flex-col items-center sm:items-start gap-2">
                        ...
                        <button  className={`text-xl px-4 py-2 rounded-lg ${
